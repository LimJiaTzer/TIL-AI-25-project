...
def main():
    ...
    results = []
    for batch_iterable in tqdm(batch_generator, total=math.ceil(len(instances) / BATCH_SIZE)):
        batch_list = list(batch_iterable)
        payload_dict = {"instances": batch_list}
        
        response = requests.post("http://localhost:5001/asr",
                                        json=payload_dict,  # Pass the Python dictionary directly
                                        timeout=60)
